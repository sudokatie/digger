/**
 * Daily Challenge System for Digger
 *
...
const DAILY_STORAGE_KEY = 'digger_daily_leaderboard';
...
export class DailyLeaderboard {
  ...
  static recordScore(
    ...
    entries.push({ name, score, level, goldCollected, timeSeconds, completedAt: now });
...
/**
 * Select a random subset of levels for today's daily challenge.
 * The daily always has 3 levels, selected from available levels.
 */
export function getDailyLevelIds(totalLevels: number): number[] {
  const rng = new SeededRNG(todaySeed());
  
  // Create array of level IDs (1 to totalLevels)
