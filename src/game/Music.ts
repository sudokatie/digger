/**
 * Music system for Digger using Web Audio API.
 * Generates chase-themed, action chiptune background music.
 */
...
class MusicSystem {
  ...
  private playNote(freq: number, startTime: number, duration: number, vol: number = 1): void {
    ...
    // Square wave for classic platform game feel
    osc.type = 'square';
  ...
  // Gameplay track - chase-themed, action platformer feel
  private getGameplayNotes(): Note[] {
    ...
    // Chase and escape melody (8 bars)
    const melody = [
      // Bar 1-2: Running theme
      { note: 'E4', dur: eighth },
      ...
      { note: 'G4', dur: eighth },
      { note: 'E4', dur: beat },
      // Bar 5-6: Escape attempt
      { note: 'D4', dur: sixteenth },
      { note: 'E4', dur: sixteenth },
      { note: 'G4', dur: sixteenth },
      { note: 'A4', dur: sixteenth },
      { note: 'B4', dur: eighth },
  ...
  private getMenuNotes(): Note[] {
    ...
    return [
      ...
      { note: 'A4', dur: beat, volume: 0.5 },
      { note: 'G4', dur: beat * 2, volume: 0.5 },
  ...
  private getGameoverNotes(): Note[] {
    ...
    return [
      ...
      { note: 'B3', dur: beat, volume: 0.4 },
      { note: 'A3', dur: beat * 2, volume: 0.3 },
    ].map(n => ({ frequency: this.noteToFreq(n.note), duration: n.dur, volume: n.volume }));
  }
